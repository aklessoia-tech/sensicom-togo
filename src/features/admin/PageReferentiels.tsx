      ...
      {onglet === 'profiles' ? (
        <div className="card md:overflow-x-auto">
          {profiles.length === 0 ? (
            ...
            <>
              {/* Six colonnes dont deux menus déroulants ne tiennent pas sur un
                  téléphone. Chaque compte occupe alors un bloc, avec les mêmes
                  contrôles empilés. */}
                  ...
                  <li key={p.id} className="space-y-2.5 py-3.5">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <button
                        ...
                        className={
                          p.actif
                            ? 'shrink-0 rounded-full bg-emerald-100 px-2.5 py-1 text-[11px] font-semibold text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200'
                            : 'shrink-0 rounded-full bg-slate-200 px-2.5 py-1 text-[11px] font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300'
                        }
                      >
                    ...
                    <div className="flex gap-2">
                      <input
                        className="input !w-20 !py-2 !text-[13px] font-mono uppercase"
                      ...
                      />
                      <select
                        aria-label={`Rôle de ${p.nom_affichage}`}
        ...
        <div className="card md:overflow-x-auto">
          {lignes.length === 0 ? (
              ...
              <ul className="filets md:hidden">
                {lignes.map((l) => (
                  <li key={l.id} className="flex items-center gap-3 py-3">
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[13.5px] font-semibold">{l.cellules[0]}</p>
                      <p className="truncate text-[11.5px] text-slate-500 dark:text-slate-400">
                        {l.cellules.slice(1).filter(Boolean).join(' · ')}
                      </p>
                    </div>
                    <button
                      onClick={() => void supprimer(l.id)}
                      disabled={!isSupabaseConfigured}
                      className="shrink-0 text-xs font-semibold text-red-600 hover:underline disabled:opacity-40 dark:text-red-400"
                    >
                      Supprimer
                    </button>
                  </li>
            ...
            <table className="hidden w-full min-w-[480px] text-sm md:table">
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {lignes.map((l) => (
                  <tr key={l.id}>
                    {l.cellules.map((c, i) => (
